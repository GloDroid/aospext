...
cleanup: ## Cleanup
	...
	bash -c "(shopt -s dotglob nullglob && rm -rf $(OUT_SRC_DIR)/*)"
	bash -c "(shopt -s dotglob nullglob && rm -rf $(OUT_INSTALL_DIR)/*)"
	bash -c "(shopt -s dotglob nullglob && rm -rf $(OUT_BUILD_DIR)/*)"
...
copy: ## Copy sources into intermediate directory
	@echo Copying...
	mkdir -p $(OUT_SRC_DIR)
	bash -c "(shopt -s dotglob nullglob && rsync -ar $(SRC_DIR)/* $(OUT_SRC_DIR))"
